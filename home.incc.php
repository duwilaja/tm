<?php
/*graph data sources*/
$sql="select s as label, count(s) as value from tm_tickets where datediff(date(now()),date(dt))<=30";
$rs=exec_qry($conn,$sql." group by label");
$donut_status=json_encode(fetch_alla($rs));

$sql="select st as label, count(st) as value from tm_tickets where typ in $homewidget and grp='link' and datediff(date(now()),date(dt))<=30";
$rs=exec_qry($conn,$sql." group by label");
$donut_customer=json_encode(fetch_alla($rs));

$sql="select typ as label, count(typ) as value from tm_tickets t  where typ in $homewidget2 and grp='jarkom' and datediff(date(now()),date(dt))<=30";
$rs=exec_qry($conn,$sql." group by label");
...
$sql="select date(dt) as y, count(rowid) as a from tm_tickets where datediff(date(now()),date(dt))<=7";
...
function loadRange(){
	...
	$("#reportrange").daterangepicker({                    
            ...
            endDate: moment()            
			  ...
			  getBar(start.format('YYYY-MM-DD'),end.format('YYYY-MM-DD'));
              $('#reportrange span').html(start.format('D MMMM YYYY') + ' - ' + end.format('D MMMM YYYY'));
        });
	$("#reportrange span").html(moment().subtract('days', 6).format('D MMMM YYYY') + ' - ' + moment().format('D MMMM YYYY'));
}
...
function getRunTxt(){
	$.ajax({
		...
		success: function(data){
			...
			if(markui!='') {$(".scroll-left").html("<p>"+markui+"</p>");}
			else{ $(".scroll-left").html(mywelcome); }
		}
	});
	setTimeout(getRunTxt,300*1000);
}

function getCount(){
	$.ajax({
		type: 'GET',
		url: 'home.get<?php echo $env?>',
		success: function(data){
			var resall=JSON.parse(data);
			var res=resall[0];
			$("#total").html(res['total']);
			$("#open").html(res['open']);
			$("#solved").html(res['solved']);
			$("#pending").html(res['pending']);
			$("#progr").html(res['progr']);
			
			res=resall[1];
			$("#total2").html(res['total']);
			$("#open2").html(res['open']);
			$("#solved2").html(res['solved']);
			$("#pending2").html(res['pending']);
			$("#closed2").html(res['closed']);
			
			res=resall[2];
			$("#total3").html(res['total']);
			$("#open3").html(res['open']);
			$("#solved3").html(res['solved']);
			$("#pending3").html(res['pending']);
			
			$("#trlog").removeClass("informer-danger").removeClass("informer-success");
			if(resall[3]>0){
				$("#trlog").addClass("informer-success").html(resall[3]);
			}else{
				$("#trlog").addClass("informer-danger").html("0");
			}
	...
	setTimeout(getCount,30*1000);
}

function getBar(dtf,dtt){
	$.ajax({
		type: 'POST',
		url: 'datajson<?php echo $env?>',
		data: {q:'bar',id:dtf,idx:dtt},
		success: function(data){
...
function runChart(){
    mybar = Morris.Bar({
        element: 'morris-bar-daily',
        data: <?php echo $bar_daily ?>,/*[
            { y: '2006', a: 100, b: 90 },
            { y: '2007', a: 75,  b: 65 },
            { y: '2008', a: 50,  b: 40 },
            { y: '2009', a: 75,  b: 65 },
	...
	Morris.Bar({
        element: 'morris-donut-sla',
        data: <?php echo $donut_sla?>,/*[
            {label: "Download Sales", value: 12},
            {label: "In-Store Sales", value: 30},
            {label: "Mail-Order Sales", value: 20}
        ],
        colors: [randomColor(),randomColor(),randomColor(),randomColor(),randomColor(),randomColor(),randomColor()]*/
		xkey: 'label',
        ykeys: ['value'],
        labels: ['Tickets'],
        barColors: [randomColor()],
	...
	Morris.Bar({
        element: 'morris-donut-customer',
        data: <?php echo $donut_customer?>,/*[
            {label: "Download Sales", value: 12},
            {label: "In-Store Sales", value: 30},
            {label: "Mail-Order Sales", value: 20}
        ],
